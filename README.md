...
## Features
- **Stance & Form Dancing**: Change stance or form and cast another spell in the **same click**. `/cast Battle Stance` followed by `/cast Charge` fires both on a single press, instead of needing one click to swap and another to act. Works for warrior stances and druid forms with any follow-up ability that isn't on the global cooldown. `[stance:N]` and `[form:N]` macro conditionals also see the change immediately, so gated macros work in one click too. On by default — `/console enableStancePatch 0` restores stock behaviour for that session.
...
- **Cursor-Targeted Macros**: `[@cursor]` and `[@playerlocation]` place ground-targeted spells without the second click — `/cast [@cursor] Blizzard`, `/cast [@playerlocation] Death and Decay`. (Class-dependent: warriors have no ground-targeted spells in stock WotLK.)
- **Unit Info API**: New Lua queries for addons and WeakAuras — `UnitIsSilenced`, `UnitIsDisarmed`, `UnitIsControlled` (fleeing, confused, stunned or pacified), `UnitOccupations` (NPC flags — vendor, repair, flightmaster…), `UnitOwner` (a pet or totem's owner name and GUID), `UnitTokenFromGUID` (a GUID back to a unit token, nameplates included) and `GetSpellBaseCooldown` (cooldown and GCD in milliseconds). The flag checks read the unit's flags directly rather than scanning for known aura IDs, so they catch custom abilities too.
