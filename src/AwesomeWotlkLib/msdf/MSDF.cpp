...
// Fork-side bridge: the fork owns the "MSDFMode" CVar (registered with its own Hooks/CVar
// types) and forwards the parsed integer value here, so this isolated library needs none of
// upstream's CVar machinery. Mirrors the original CVarHandler_MSDFMode behaviour.
void MSDF::setMode(int mode) {
	if (mode < MSDF_DISABLED) mode = MSDF_DISABLED;
