...
public:
	static bool IsFontMSDFCompatible(msdfgen::FontHandle* font) {
		...
		for (uint32_t cp = 32; cp < 127; ++cp) { if (!IsGlyphValid(font, cp)) { return false; } }
		return true;
	}
...
private:
	...
	static bool isnan_inf(double v) { return std::isnan(v) || std::isinf(v); }

	static bool isValidCoord(double v) { return !isnan_inf(v) && std::abs(v) <= MAX_COORD; }
	...
	static void flattenQuadratic(const Vec& p0, const Vec& p1, const Vec& p2, std::vector<Vec>& out, double tol, int depth = 0) {
		if (depth > 20) {
			out.push_back(p2);
			return;
		}

		const Vec m(0.25 * p0.x + 0.5 * p1.x + 0.25 * p2.x, 0.25 * p0.y + 0.5 * p1.y + 0.25 * p2.y);
	...
	static void flattenCubic(const Vec& p0, const Vec& p1, const Vec& p2, const Vec& p3, std::vector<Vec>& out, double tol, int depth = 0) {
		if (depth > 20) {
			out.push_back(p3);
			return;
		}

		const Vec m(0.125 * (p0.x + 3.0 * p1.x + 3.0 * p2.x + p3.x), 0.125 * (p0.y + 3.0 * p1.y + 3.0 * p2.y + p3.y));
	...
	static bool onSegment(const Vec& a, const Vec& b, const Vec& p) {
		...
		return std::min(a.x, b.x) - EPS <= p.x && p.x <= std::max(a.x, b.x) + EPS && std::min(a.y, b.y) - EPS <= p.y && p.y <= std::max(a.y, b.y) + EPS;
	}
		...
		for (size_t i = 0; i < n; ++i) {
			...
			for (size_t j = i + 2; j < n; ++j) {
				...
				if (segsIntersectProper(a1, a2, b1, b2)) {
					const bool shareEndpoint = (Vec(a1.x - b1.x, a1.y - b1.y).lengthSq() <= EPS * EPS) || (Vec(a1.x - b2.x, a1.y - b2.y).lengthSq() <= EPS * EPS) || (Vec(a2.x - b1.x, a2.y - b1.y).lengthSq() <= EPS * EPS) || (Vec(a2.x - b2.x, a2.y - b2.y).lengthSq() <= EPS * EPS);
	...
	static int conic_to_func_internal(const FT_Vector* control, const FT_Vector* to, void* user) {
		...
		const Vec p1(control->x, control->y);
		const Vec p2(to->x, to->y);
	...
	static int cubic_to_func_internal(const FT_Vector* c1, const FT_Vector* c2, const FT_Vector* to, void* user) {
		...
		const Vec& p0 = ctx->current.back();
		const Vec p1(c1->x, c1->y);
		const Vec p2(c2->x, c2->y);
		const Vec p3(to->x, to->y);
	...
	static bool validateOutline(const FT_Outline& outline, double tol) {
		...
		FT_Outline_Funcs funcs = {.move_to = move_to_func_internal, .line_to = line_to_func_internal, .conic_to = conic_to_func_internal, .cubic_to = cubic_to_func_internal, .shift = 0, .delta = 0};

		if (FT_Outline_Decompose(const_cast<FT_Outline*>(&outline), &funcs, &ctx) != 0) { return false; }
		if (!ctx.current.empty()) { ctx.contours.push_back(std::move(ctx.current)); }
		for (const auto& cont : ctx.contours) {
			if (cont.size() < MIN_CONTOUR_SIZE) return false;
			for (const auto& p : cont) { if (!isValidCoord(p.x) || !isValidCoord(p.y)) return false; }
	...
	static std::vector<Vec> flattenMsdfContour(const msdfgen::Contour& contour, double tol) {
		...
		for (const auto& edge : contour.edges) {
			...
			if (result.empty()) { result.emplace_back(start.x, start.y); }
			...
			if (isLinear) { result.push_back(edgePoints.back()); }
			else { for (size_t j = 1; j < edgePoints.size(); ++j) { result.push_back(edgePoints[j]); } }
		}
		return result;
	}
	...
	static bool validateResolvedShape(const msdfgen::Shape& shape, double tol) {
		...
		for (const msdfgen::Contour& contour : shape.contours) {
			...
			for (const auto& p : pts) { if (!isValidCoord(p.x) || !isValidCoord(p.y)) { return false; } }
